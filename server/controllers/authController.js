...
exports.registerStoreOwner = async (req, res) => {
    ...
    // Validate required fields
    if (!email || !password || !first_name || !contact_number) {
        return res.status(400).json({ message: 'Missing required fields: email, password, first_name, contact_number' });
    }
    ...
    try {
        // Check for duplicate email
        db.query('SELECT store_owner_id FROM store_owners WHERE email = ?', [email.trim()], async (dupErr, dupRows) => {
            if (dupErr) return res.status(500).json({ error: dupErr.message });
            if (dupRows && dupRows.length > 0) {
                return res.status(409).json({ message: 'An account with this email already exists.' });
            }
...
exports.login = async (req, res) => {
    ...
    try {
        const tables = ['admins','technicians','customers','store_owners'];
        let found = null;
        ...
        // if technician require approval; if customer require active status; if store_owner require approval
        ...
        const idField = ['id','customer_id','admin_id','technician_id','store_owner_id'].find(f => Object.prototype.hasOwnProperty.call(user, f));
