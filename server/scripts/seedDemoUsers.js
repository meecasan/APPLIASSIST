...
const seedDemoUsers = async () => {
  try {
    ...
    // =========================================================================
    // SEED STORE OWNER USERS
    // =========================================================================
    console.log('🏪 Creating Store Owner Users...');
    ...
    for (const owner of storeOwnerQueries) {
      const ownerQuery = `
        INSERT INTO store_owners (first_name, last_name, email, password, contact_number, address, account_status)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
          password = VALUES(password),
          account_status = VALUES(account_status),
      ...
      db.query(
        ownerQuery,
        [owner.firstName, owner.lastName, owner.email, demoHashedPassword, '555-0002', 'Naga City, Camarines Sur', owner.status],
        (err, result) => {
          if (err) {
            console.error(`  ❌ Error creating store owner ${owner.email}:`, err.message);
          } else {
            console.log(`  ✓ Store Owner: ${owner.email} (status: ${owner.status})`);
          }
        }
      );
    }
    ...
    console.log('  - Shop Owner (Approved): shop@example.com');
    console.log('  - Shop Owner (Pending): newshop@example.com');
