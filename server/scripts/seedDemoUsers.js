...
const seedDemoUsers = async () => {
  try {
    ...
    for (const owner of storeOwnerQueries) {
      ...
      db.query(
        ownerQuery,
        [owner.firstName, owner.lastName, owner.email, demoHashedPassword, '555-0004', '456 Shop Street, Naga City', owner.status],
        (err, result) => {
          if (err) {
    ...
    console.log('  - Store Owner (Approved): owner@example.com');
