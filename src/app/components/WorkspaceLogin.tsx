...
export function WorkspaceLogin({ onLogin, onNavigateToHome, onNavigateToJoinUs }: WorkspaceLoginProps) {
  const [formData, setFormData] = useState({
    email: '',
    password: '',
    role: 'technician' as 'technician' | 'shop-owner',
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1E2F4F] via-[#2a4066] to-[#1E2F4F] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          {/* Role Selection */}
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-900 mb-3" style={{ fontFamily: 'Poppins, sans-serif' }}>
              Login as
            </label>
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center justify-center space-x-2 px-4 py-3 rounded-lg border-2 transition-all ${
                  formData.role === 'technician'
                    ? 'border-[#1E2F4F] bg-blue-50 text-[#1E2F4F]'
              ...
              <button
                type="button"
                onClick={() => setFormData({ ...formData, role: 'shop-owner' })}
                className={`flex items-center justify-center space-x-2 px-4 py-3 rounded-lg border-2 transition-all ${
                  formData.role === 'shop-owner'
                    ? 'border-[#1E2F4F] bg-blue-50 text-[#1E2F4F]'
                    : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                }`}
                style={{ fontFamily: 'Poppins, sans-serif' }}
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-4">
          ...
          <div className="space-y-1 text-xs text-blue-200" style={{ fontFamily: 'Manrope, sans-serif' }}>
            ...
            <p>Shop Owner: shop@example.com / password123</p>
          </div>
        </div>
      </div>
    </div>
