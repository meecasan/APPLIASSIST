  ...
  useEffect(() => {
    const loadServices = async () => {
      try {
        ...
        // Group services by type/category and count technicians
        const categoryMap: Record<string, { count: number; services: any[] }> = {};
        
        (data || []).forEach((service: any) => {
          const serviceType = service.appliance_type || 'Other';
          if (!categoryMap[serviceType]) {
            categoryMap[serviceType] = { count: 0, services: [] };
          }
          categoryMap[serviceType].count++;
          categoryMap[serviceType].services.push(service);
        });

        // Transform into ServiceCategory format
        ...
        setServiceCategories(categories.length > 0 ? categories : getDefaultCategories());
      } catch (err) {
